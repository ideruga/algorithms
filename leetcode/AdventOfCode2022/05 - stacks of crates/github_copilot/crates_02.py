        ...
        if line.replace(" ", "").strip().isdigit():
            ...
            for line in input_lines[i + 2:]:
                if line.startswith("move"):
                    ...
                    count = int(count)
                    # copy <count> crates from <source> to <destination> in order they appear in <source>:
                    crates = []
                    for _ in range(count):
                        crates.append(stacks[int(source) - 1].pop())
                    crates.reverse()
                    stacks[int(destination) - 1].extend(crates)
